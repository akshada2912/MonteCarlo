...
Vector3f SampleUniformHemisphere(float u1, float u2)
{
    ...
    // float phi = (u2 * 2 * M_PI) - M_PI;

    // // Convert to Cartesian coordinates
    // float x = std::sin(theta) * std::cos(phi);
    // float y = std::sin(theta) * std::sin(phi);
    // float z = std::cos(theta);  // Ensure z is non-negative for upper hemisphere
    ...
    if (node.primCount != 0)
    {
        ...
        for (uint32_t i = 0; i < node.primCount; i++)
        {
            ...
            if (siIntermediate.t <= ray.t && siIntermediate.didIntersect)
            {
                ...
                // Vector3f w = Normalize(si.n);
                // Vector3f u = Normalize(Cross(std::abs(w.x) > 0 ? (std::abs(w.y) > 0 ? Vector3f(0, 1, 0) : Vector3f(0, 1, 0)) : Vector3f(1, 0, 1), w)); // perpendicular to w cross w
                // Vector3f v = Normalize(Cross(w, u));
                // // Vector3f u = Normalize(Cross(std::abs(w.x) > 0 ? Vector3f(0, 1, 0) : Vector3f(1, 0, 0), w)); // perpendicular to w cross w
                // // Vector3f u = Normalize(Cross(std::abs(w.x) == 0 ? Vector3f(1, 0, 0) : ( std::abs(w.y) == 0 ? Vector3f(0, 1, 0) : Vector3f(0, 0, 1)), w));

                // if (w.x != 0 && w.y != 0 && w.z == 0)
                // {
                //     u = Vector3f(0, 0, 1);
                //     v = Vector3f(w.y, -w.x, 0);
                // }

                // if (w.x == 0 && w.y != 0 && w.z != 0)
                // {
                //     u = Vector3f(1, 0, 0);
                //     v = Vector3f(0, w.z, -w.x);
                // }

                // if (w.x != 0 && w.y == 0 && w.z != 0)
                // {
                //     u = Vector3f(0, 1, 0);
                //     v = Vector3f(-w.z, 0, w.x);
                // }
                ...
                // if (w.x != 0 && w.y != 0 && w.z != 0)
                // {
                //     Vector3f v1 = {1.0f, 0.0f, 0.0f}; // First arbitrary vector
                //     Vector3f v2 = {0.0f, 1.0f, 0.0f}; // Second arbitrary vector

                //     // Step 3: Gram-Schmidt Process
                //     // Step 3.1: Make v1 orthogonal to n
                //     float dot_v1_n = Dot(v1, w);
                //     Vector3f v1_ortho = v1 - dot_v1_n * w;

                //     // Step 3.2: Make v2 orthogonal to both n and v1_ortho
                //     float dot_v2_n = Dot(v2, w);
                //     float dot_v2_v1_ortho = Dot(v2, v1_ortho);
                //     Vector3f v2_ortho = v2 - dot_v2_n * w - dot_v2_v1_ortho * v1_ortho;

                //     // Step 4: Normalize v1_ortho and v2_ortho
                //     float invLength_v1_ortho = 1.0f / sqrt(v1_ortho.x * v1_ortho.x + v1_ortho.y * v1_ortho.y + v1_ortho.z * v1_ortho.z);
                //     float invLength_v2_ortho = 1.0f / sqrt(v2_ortho.x * v2_ortho.x + v2_ortho.y * v2_ortho.y + v2_ortho.z * v2_ortho.z);
                //     v1_ortho.x *= invLength_v1_ortho;
                //     v1_ortho.y *= invLength_v1_ortho;
                //     v1_ortho.z *= invLength_v1_ortho;
                //     v2_ortho.x *= invLength_v2_ortho;
                //     v2_ortho.y *= invLength_v2_ortho;
                //     v2_ortho.z *= invLength_v2_ortho;
                ...
                si.b=Normalize(Cross(ray.d,si.c));
                si.a=Normalize(Cross(si.c,si.b));

                //  if(w.x==0 && w.y!=0 && w.z!=0)
                // u=Vector3f(0,w.z,-w.y);

                //  if(w.x!=0 && w.y==0 && w.z!=0)
                // u=Vector3f(0,w.z,0);

                // if(w.x>0.1 && w.y>0.1 && w.z>0.1)
                // Vector3f form=w;
                // if(!w.x)
                // form=Vector3f(1,0,0);
                ...
                // Vector3f u=Normalize(Cross(w,form));
                // if (std::abs(w.x) <0.1)
                // {
                //     // Choose the z-axis as the reference vector if w is not parallel to the z-axis
                //     u = Normalize(Cross(Vector3f(1, ray.d.y, ray.d.z), w));
                // }
                // else if (std::abs(w.z) <0.1)
                // {
                //     // Choose the x-axis as the reference vector if w is parallel to the z-axis but not aligned with the x-axis
                //    u = Normalize(Cross(Vector3f(ray.d.x, 1, ray.d.z), w));
                // }
                // else if(std::abs(w.y)<0.1)
                // {
                //     // Choose the y-axis as the reference vector if w is parallel to the x-axis or the y-axis
                //    u = Normalize(Cross(Vector3f(ray.d.x, ray.d.y, 1), w));
                // }
